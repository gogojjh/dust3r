...
class Dust3RPipeline:
	...
	def process_images(self, img1_path, img2_path, img3_path, save_conf_path, resize=512):
		"""Process pair of images and return aligned scene"""
		# Load and prepare images
		imgs_path = [str(img1_path), str(img2_path), str(img3_path)]
		...
		axes[1].set_title(f'Confidence Map {1}')
		fig.colorbar(im, ax=axes[1])
		plt.savefig(os.path.join(save_conf_path, 'conf_map.png'))

		W_i = scene.weight_i["0_1"].detach().cpu().numpy()
		W_j = scene.weight_j["0_1"].detach().cpu().numpy()
		fig, axes = plt.subplots(1, 2, figsize=(10, 5))
		im = axes[0].imshow(W_i, cmap='jet')
		axes[0].set_title(f'Weight Map {0}')
		fig.colorbar(im, ax=axes[0])
		im = axes[1].imshow(W_j, cmap='jet')
		axes[1].set_title(f'Weight Map {1}')
		fig.colorbar(im, ax=axes[1])
		plt.savefig(os.path.join(save_conf_path, 'calib_conf_map.png'))
...
def main():
	...
	parser.add_argument('--lora_path', default=None, help='Path to LoRA weights (optional)')
	...
	parser.add_argument('--img3', required=True, help='Path to third image')
	...
	# Process images
	try:
		scene = pipeline.process_images(args.img1, args.img2, args.img3, args.save_conf_path)
