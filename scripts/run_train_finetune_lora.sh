...
TRAIN_ARGS=(
    "--train_dataset" "100 @ MapFree(split='test', ROOT='$DATASET_PATH/', aug_crop=16, resolution=[(512, 288)])"
    "--test_dataset" "100 @ MapFree(split='test', ROOT='$DATASET_PATH/', resolution=(512, 288), seed=777)"
...
for SUFFIX in "${SUFFIXES[@]}"; do
    ...
    # Training command
    OUTPUT_DIR="${MODEL_WEIGHTS_DIR}/dust3r_${DATASET_NAME}_${SCENE}_512dpt_calib_ftlora_${SUFFIX}"
    python train.py "${TRAIN_ARGS[@]}" --output_dir "$OUTPUT_DIR"
    
    # Post-processing
    rm "$OUTPUT_DIR"/*.pth
    cp "$OUTPUT_DIR/lora.pt" "${DATASET_PATH}/finetune/weights/lora_${SCENE}_${SUFFIX}.pt"
    rm "${DATASET_PATH}/finetune/mapfree_pairs.npy"
